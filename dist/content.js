...
${a}`;t.addEventListener("mouseenter",c=>{if(!h)return;b();const g=M(m);x=g,E=t,L(g,c),D(r),w(c),z({metricName:y,initialValue:n[0]??null,finalValue:n[n.length-1]??null,compoundRate:o})}),t.addEventListener("mousemove",c=>{h&&(x&&E===t&&L(x,c),w(c))}),t.addEventListener("mouseleave",()=>{b()})}}})})();
